...
computeMetrics = function(method,allSettings){
  ...
  for (hyper in 1:1) {
    ...
    for (setting in allSettings) {
      signal = 5
      noisetype = setting[1]
      simu_n = as.numeric(setting[2])
      px = as.numeric(setting[3])
      py = as.numeric(setting[4])
      # rparam = allparam[which(allSignal==signal)]
      # for (prop in allprop)
        {
        # for (simu_n in allN) {
...
allSettings = list(c("Dirichlet",30,30,40),c("Dirichlet",40,30,40),c("Dirichlet",50,30,40),
                c("Sparse",100,100,100),c("Sparse",150,100,100))
