...
computeAccuracy = function(allMethod,allhyper){
...
computeMetrics = function(method,allhyper){
...
# allMethod = c("VI")
allMethod = c("compDAG_hyper")#
# allSettings = list(c("Cyclic",2),c("Cyclic",3),c("Cyclic",5))
# allNoisetype = c("zero-inflated")
# px = 30
# py = 40
allhyper = matrix(0,6,3)
# allhyper[1,] = c(1,1,0.025)
allhyper[1,] = c(1,0.1,0.025)
# allprop = c(0.2,0.4,0.6)
allhyper[2,] = c(10,1,0.025)
allhyper[3,] = c(1,0.1,0.025)
allhyper[4,] = c(1,10,0.025)
allhyper[5,] = c(1,1,0.01)
allhyper[6,] = c(1,1,0.1)
# pdir = 0.025
# allP = c(10)
# allP = c(20,30)
# simu_n = 500
...
finalList = computeAccuracy(allMethod,allhyper)
...
for (method in allMethod) {
  for (thres in threshold) {
    finalList = computeMetrics(method,allhyper)
