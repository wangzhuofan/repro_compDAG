...
simu_plot <- function(noisetype,samplesize){
  ...
  #             parse = TRUE, size = 6, hjust = -1) +
  #   theme_void()
  gp <- ggpubr::ggarrange(plotlist = list(p0,p1,p2,p3,p4,p5),ncol = 6,nrow = 1,common.legend = TRUE,legend = "right",labels = c("(A)","(B)","","","",""),font.label = list(size=20))
  # annotate_figure(gp,
  ...
  # )
  filename = paste0("./output/Figure1-2_S.1-2/simu_",noisetype,"_",samplesize,".pdf")
