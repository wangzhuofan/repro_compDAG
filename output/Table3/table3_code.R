...
for (signal in c(0.3,0.5,1,3,5)) {
  for (n in c(100,150)) {
    ...
    cat(paste0('The results of compDAG in the first simulation setting when n = ',n,' and pi = ',signal,' are: TPR:',mean(tpr),'(',sd(tpr),'); FDR:',mean(fdr),'(',sd(fdr),'); MCC:',mean(mcc),'(',sd(mcc),').'))
    cat(paste0('The results of PC in the first simulation setting when n = ',n,' and pi = ',signal,' are: TPR:',mean(tpr_pc),'(',sd(tpr_pc),'); FDR:',mean(fdr_pc),'(',sd(fdr_pc),'); MCC:',mean(mcc_pc),'(',sd(mcc_pc),').'))
  }
}